...
QBCore.Functions.CreateCallback('gb_jobselect:server:getJobs', function(source, cb)
    local src = source
...
RegisterNetEvent('gb_jobselect:server:changeJob')
AddEventHandler('gb_jobselect:server:changeJob', function(job, grade)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local JobInfo = QBCore.Shared.Jobs[job]
    if job == 'police' or job == 'ambulance' then
        Player.Functions.SetJob(job, grade)
        TriggerClientEvent('QBCore:Notify', src, 'TU TRABAJO AHORA ES '..JobInfo.label)
    else
        Player.Functions.SetJob(job, grade)
        Player.Functions.SetJobDuty(true)
        TriggerClientEvent("QBCore:Client:SetDuty", src, true)
        TriggerClientEvent('QBCore:Notify', src, 'TU TRABAJO AHORA ES '..JobInfo.label .. ', AHORA ESTAS EN SERVICIO')
    end
end)

RegisterNetEvent('gb_jobselect:server:newJob')
AddEventHandler('gb_jobselect:server:newJob', function(jobInfo)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local cid = Player.PlayerData.citizenid
    local canAdd = true
...
RegisterNetEvent('gb_jobselect:server:deleteJob')
AddEventHandler('gb_jobselect:server:deleteJob', function(job)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local cid = Player.PlayerData.citizenid
